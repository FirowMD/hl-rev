<script lang="ts">
  ...
  import { globalToEdit, mainPanelTab } from './types';
  ...
  let menuX = 0;
  let menuY = 0;
  let selectedGlobalIdx: number | null = null;
  let selectedGlobalName: string | null = null;
  ...
  function handleContextMenu(e: MouseEvent, idx: number) {
    ...
    selectedGlobalName = name + idxStr;
...
<div class="h-full">
  ...
  {#if showMenu && selectedGlobalIdx !== null}
    <div
    ...
    >
      <button class="block w-full text-left p-1 hover:bg-primary-400/30 rounded"
              role="menuitem"
              tabindex="0"
              onclick={copyGlobalMenu}
              onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') copyGlobalMenu(); }}>
        Copy
      </button>
      <button class="block w-full text-left p-1 hover:bg-primary-400/30 rounded"
              role="menuitem"
              tabindex="0"
              onclick={editGlobalMenu}
              onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') editGlobalMenu(); }}>
        Edit
      </button>
      <button class="block w-full text-left p-1 hover:bg-error-400/30 text-error-400 rounded"
              role="menuitem"
              tabindex="0"
              onclick={deleteGlobalMenu}
              onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') deleteGlobalMenu(); }}>
        Delete
      </button>
    </div>
  {/if}
