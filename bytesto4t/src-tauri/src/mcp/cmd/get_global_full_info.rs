...
#[derive(Clone)]
pub struct GetGlobalFullInfoHandler {
    pub app_handle: AppHandle,
}

#[async_trait]
impl ToolHandler for GetGlobalFullInfoHandler {
    async fn call(&self, arguments: HashMap<String, Value>) -> McpResult<CallToolResult> {
        ...
        let ty_ref = bytecode.globals[index];
        let out = json!({
            "index": ty_ref.0
        });
...
pub async fn register(server: &mut McpServer, app_handle: AppHandle) -> McpResult<()> {
    ...
    server
        .add_tool(
            "get_global_full_info".to_string(),
            Some("Get global info".to_string()),
            json!({
                "type": "object",
            ...
            GetGlobalFullInfoHandler { app_handle: ah },
        )
        .await?;
    Ok(())
}