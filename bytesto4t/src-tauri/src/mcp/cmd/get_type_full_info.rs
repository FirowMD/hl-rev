...
#[derive(Clone)]
pub struct GetTypeFullInfoHandler {
    pub app_handle: AppHandle,
}

#[async_trait]
impl ToolHandler for GetTypeFullInfoHandler {
    async fn call(&self, arguments: HashMap<String, Value>) -> McpResult<CallToolResult> {
        ...
        let t = &bytecode.types[index];
        let json_str = t.to_json().map_err(|e| McpError::Internal(e.to_string()))?;
...
pub async fn register(server: &mut McpServer, app_handle: AppHandle) -> McpResult<()> {
    ...
    server
        .add_tool(
            "get_type_full_info".to_string(),
            Some("Get full type info as JSON".to_string()),
            json!({
                "type": "object",
            ...
            GetTypeFullInfoHandler { app_handle: ah },
        )
        .await?;
    Ok(())
}