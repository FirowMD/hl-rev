...
#[async_trait]
impl ToolHandler for ImportFunctionJsonHandler {
...
pub async fn register(server: &mut McpServer, app_handle: AppHandle) -> McpResult<()> {
    ...
    server
        .add_tool(
            "import_function_json".to_string(),
            Some("Import function from JSON file".to_string()),
            json!({
                "type": "object",
            ...
            ImportFunctionJsonHandler { app_handle: ah },
        )
        .await?;
    Ok(())
}